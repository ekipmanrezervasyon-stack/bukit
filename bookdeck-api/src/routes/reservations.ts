...
const SECURITY_APPROVAL_NOTE =
  "Öğrencimizin ekteki ekipmanlarla kampüs dışı çıkışı uygundur. Ekipman fazlalığına göre takdiri güvenlik amirliğine ait olmak üzere Cami Kapı Stüdyo Ana kapısına kadar taşıma amaçlı araç girişine müsaade edilmesini rica ederiz. İletişim Fakültesi Stüdyolar Birimi (Acil Durumlarda Taylan Yuva'dan bilig alınabilir)";
...
const sendSecurityEquipmentCheckoutPdfCopyEmail = async (
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;line-height:1.45;color:#111827">
      ...
      <p>${escapeHtml(SECURITY_APPROVAL_NOTE)}</p>
...
export const reservationRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/admin/studio-reservations/approve", { preHandler: requireRoles(ADMIN_ROLES) }, async (req, reply) => {
    ...
    try {
      ...
      if (!sync.ok) {
        req.log.warn({ reservationId: String(updated.id || id), skipped: String(sync.skipped || "") }, "sync approved studio reservation skipped");
      }
    } catch (e) {
    ...
    if (st.data) {
      ...
      if (String(stRow.status || "") === STUDIO_APPROVED_STATUS) {
        try {
          ...
          if (!sync.ok) {
            req.log.warn({ reservationId: String(stRow.id || id), skipped: String(sync.skipped || "") }, "sync studio reservation skipped on extend");
          }
        } catch (e) {
    ...
    if (st.data) {
      ...
      if (String(stRow.status || "") === STUDIO_APPROVED_STATUS) {
        try {
          ...
          if (!sync.ok) {
            req.log.warn({ reservationId: String(stRow.id || id), skipped: String(sync.skipped || "") }, "sync studio reservation skipped on transfer");
          }
        } catch (e) {
  ...
  const updateEquipmentMetaHandler = async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (nowAvailable) {
      const activeReservation = await supabaseAdmin
        .from("equipment_reservations")
        .select("id,status,requester_profile_id,start_at,end_at")
        .eq("equipment_item_id", id)
        .in("status", EQUIPMENT_ACTIVE_RES_STATUSES)
        .limit(1)
        .maybeSingle();
        ...
        req.log.warn(
          {
            ...
            reservationRequesterProfileId: String((activeReservation.data as Record<string, unknown>).requester_profile_id || ""),
